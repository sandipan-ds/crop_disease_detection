...
│
├── hypothesis_testing/           # Offline statistical evaluation (ranking, heatmaps, bootstrap)
│
├── gradcam_view/                 # Model explainability visualizations (attention maps)
│
...
To build trust with farmers and agronomists, it is not enough for a model to be accurate; we must also verify *why* it makes a decision. We use **Grad-CAM (Gradient-weighted Class Activation Mapping)** to visualize where each model focuses its attention on the plant leaf when diagnosing a disease.

### 🔍 Key Interpretability Insights
* **ResNet-152 (Our Champion):** Displays extremely sharp, localized attention maps. It focuses directly on the physical symptoms of the disease, such as fungal lesions, rust spots, and leaf curling.
* **MobileNetV3 & Swin-Base:** Show reasonable focus on the relevant areas, though Swin-Base (being a hierarchical transformer) displays more blocky/patch-based attention patterns.
* **ViT (B/16) Limitations:** Pure Vision Transformers distribute attention globally across the entire leaf. Because of this, traditional Grad-CAM heatmaps for ViT appear very diffuse or transparent, showing that it lacks localized feature attention in the convolutional sense.

### 🖼️ Grad-CAM Visualizations

#### 1. Detailed Model Comparison
This visualization shows side-by-side attention comparisons across the architectures for specific disease samples.
![Grad-CAM Comparison](gradcam_view/gradcam_comparison.png)

#### 2. 10-Class Diverse Diagnostic Grid
A comprehensive $10 \times 5$ grid displaying the original leaf images alongside the Grad-CAM heatmaps generated by MobileNetV3, ResNet-152, Swin-Base, and ViT (B/16).
![Grad-CAM Diagnostic Grid](gradcam_view/gradcam_comparison_grid.png)
